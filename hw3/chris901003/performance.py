...
'''
    ...
    with open('performance.txt', 'w') as f:
        f.write(f'Start multiply_naive (repeat={repeat}), take min = ')
        naivesec = minsec = min(naive.repeat(repeat=repeat, number=1))
        f.write(f'{minsec} seconds\n')

        f.write(f'Start multiply_tile (repeat={repeat}), take min = ')
        tilesec = minsec = min(tile.repeat(repeat=repeat, number=1))
        f.write(f'{minsec} seconds\n')

        f.write(f'Start multiply_mkl (repeat={repeat}), take min = ')
        mklsec = minsec = min(mkl.repeat(repeat=repeat, number=1))
        f.write(f'{minsec} seconds\n')

        f.write('tile speed-up over naive: %g x\n' % (naivesec/tilesec))
        f.write('MKL speed-up over naive: %g x\n' % (naivesec/mklsec))
