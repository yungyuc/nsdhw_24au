...
public:
    size_t row, column;
    OurAllocator<double> allocator;
    std::vector<double, OurAllocator<double>> matrix_data;

    Matrix(size_t r, size_t c) : row(r), column(c) { 
        matrix_data = std::vector<double, OurAllocator<double>>(allocator);
        matrix_data.resize(r * c);
    }

    void SetMatrixData(std::vector<double, OurAllocator<double>> data) {
