...
template<typename T>
size_t CustomAllocator<T>::current = 0;
...
Matrix::Matrix(size_t _row, size_t _col):buffer(_row * _col, 0.0){
    this->nrow = _row;
    this->ncol = _col;
    //this->buffer = new double[_row * _col];
    //std::fill(this->buffer, this->buffer + _row * _col, 0.0);
    //this->buffer = std::vector<double>(_row * _col, 0.0);
}
...
PYBIND11_MODULE(_matrix, m){
    pybind11::class_<Matrix>(m, "Matrix")
        ...
        .def_property("array", [](Matrix &self) {
            return pybind11::array_t<double>({self.get_row(), self.get_col()}, self.buffer_ref(), pybind11::cast(self));
        }, nullptr);
    m.def("multiply_naive", &multiply_naive);
    m.def("multiply_mkl", &multiply_mkl);
    m.def("multiply_tile", &multiply_tile);
    m.def("bytes", []() { return CustomAllocator<double>::get_current(); });
